...
// ====== mostrar y ocultar formulaarios ======
    ...
    button.addEventListener('click', () => {
        ...
        if (formId === "PolizaVigente") cargarPolizasVigentes();
        if (formId === "clienteVigente") cargarClientesVigentes();
        if (formId === "porRenovar") cargarPolizasPorRenovar();
        if (formId === "SiniestrosVigentes") cargarSiniestrosVigentes();
    ...
    <td>$${Number(poliza.valor).toLocaleString("es-CO")}</td>
    <td>${poliza.estado}</td>
